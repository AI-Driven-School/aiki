#!/bin/bash
# pipeline-engine.sh - Closed-loop quality pipeline for aiki
#
...
escalate_to_github() {
    ...
    local title="[aiki] Auto-fix failed: ${phase} phase for '${feature}'"
    local body
    body=$(cat <<GHEOF
...
---
*Auto-generated by aiki pipeline-engine*
GHEOF
)
...
run_pipeline() {
    ...
    # Trap to clean up phase file on exit (chain with existing traps from tui.sh)
    local phase_file="${PROJECT_DIR}/.claude/.pipeline_phase"
    mkdir -p "$(dirname "$phase_file")"
    _pipe_existing_exit_trap=$(trap -p EXIT | sed "s/^trap -- '//;s/' EXIT$//")
    if [ -n "$_pipe_existing_exit_trap" ]; then
        trap "rm -f '${PROJECT_DIR}/.claude/.pipeline_phase'; $_pipe_existing_exit_trap" EXIT
    else
    ...
    for phase in "${phase_list[@]}"; do
        ...
        echo ""
        if [ "${TUI_AVAILABLE:-false}" = "true" ]; then
            tui_phase_header "$(phase_name_to_number "$phase")" "$phase" "$ai" "${#phase_list[@]}"
            tui_spinner "Running ${phase}..."
        else
        ...
        if [ "$DRY_RUN" = "true" ]; then
            if [ "${TUI_AVAILABLE:-false}" = "true" ]; then
                tui_spinner_stop "[DRY-RUN] skipped"
            fi
        ...
        if [ "$NO_CACHE" != "true" ] && type compute_cache_key &>/dev/null; then
            ...
            if cache_hit "$phase" "$feature_slug" "$cache_key"; then
                if [ "${TUI_AVAILABLE:-false}" = "true" ]; then
                    tui_spinner_stop "[CACHE HIT] Skipped"
                fi
        ...
        # Stop spinner after phase completes
        if [ "${TUI_AVAILABLE:-false}" = "true" ]; then
            tui_spinner_stop "${phase} completed"
        fi
    ...
    if [ "${TUI_AVAILABLE:-false}" = "true" ]; then
        ...
        # Build summary table entries
        local table_entries=()
        for i in "${!REPORT_PHASES[@]}"; do
            table_entries+=("${REPORT_PHASES[$i]}:${REPORT_RESULTS[$i]}:$(format_duration "${REPORT_DURATIONS[$i]}")")
        done
        tui_summary_table "${table_entries[@]}"

        echo ""
        echo -e "  ${BOLD}Total:${NC} $(format_duration $total_duration)"

        # TUI score gauge
        tui_score_gauge "$quality_score"
...
show_help() {
    cat << 'EOF'
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Pipeline Engine - aiki Quality Pipeline
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
